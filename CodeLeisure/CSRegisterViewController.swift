    ...
    //        }
            ...
            username.rac_textSignal().subscribeNext { (sender) in
                ...
                //两个条件决定获取验证码按钮的状态
               codeRightBtn.enabled = name.length == 11 && self.t == -1
                if name.length >= 11{
...
//                    }else{
                       ...
                       //可用监控的方式，去改变按钮的状态
                       self.timer = NSTimer.jk_scheduledTimerWithTimeInterval(1.0, block: { self.t = self.t - 1
    ...
    //        		service=User.Register&phone=18192517406&password=111111&verificationCode=1234
          registerBtn.jk_handleControlEvents(UIControlEvents.TouchUpInside) { (sender) in
            ...
            NSNotificationCenter.defaultCenter().rac_addObserverForName(UIKeyboardWillChangeFrameNotification, object: nil).subscribeNext { (noti) in
                ...
                //用SnapKit给注册按钮做一个动画
                //如果需要更改一个视图的约束，需要snp_update
                //被update约束，必须是前面已经make过的
